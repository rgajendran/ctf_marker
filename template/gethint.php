...
if(isset($_POST['cid']) && isset($_POST['team']) && isset($_POST['vm'])){
	$hintOpen = Array();
	$hintClose = Array();
	$HO = Array();
	$HC = Array();
					...
					while($int1_row = mysqli_fetch_assoc($int1_result)){
						...
						if($update_points_sql){
							...
							if($update_hint_status){
								...
								if($act_update){
									include 'time.php';
									$log_sql = mysqli_query($connection, "INSERT INTO logger (DATE, TEAM, LOG) VALUES ('$fdate','$tem','[$vm] Hint unlocked - [$hinttext] - [POINTS : -$points]')");
									if($log_sql){
										$hintOpen[] = "$hinttext";
									}else{
